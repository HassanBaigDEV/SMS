...
const AcademicProfile = ({route, navigation}) => {
  ...
  const formattedDate = user.dateOfAdmission
    ? user.dateOfAdmission.substring(0, 10)
    : '';
  ...
  console.log(remarks);
  const dottedRemarks =
    remarks.charAt(remarks.length - 1) !== '.' ? remarks + '.' : remarks;
  const capitalizedRemarks = dottedRemarks
    .split(' ')
          ...
          <>
            ...
            <View style={styles.userInfo}>
              <View style={styles.schoolInfoContainer}>
                ...
                <View style={styles.schoolDetails}>
                  <Text style={styles.userName}>
                    {capitalizedUser.studentName} {capitalizedUser.fatherName}
                  </Text>
              ...
              <View style={styles.userInfo}>
                {renderUserInfoRow(
                  'Roll Number:',
                  capitalizedUser.registrationNumber,
                )}
                ...
                {renderUserInfoRow(
                  'Date of Birth:',
                  capitalizedUser.dateOfBirth,
                )}
                <View style={styles.minimalSeparator}></View>
                {renderUserInfoRow(
                  'Father Name:',
                  'Mr. ' + capitalizedUser.fatherName,
                )}
                <View style={styles.minimalSeparator}></View>
          ...
          return (
            <View style={styles.subjectContainer}>
              <Text style={styles.subjectText}>
                Subject: <Text style={styles.boldText}>{item}</Text>
              </Text>
              <Text style={styles.subjectText}>
                First Term:{' '}
                {subjectData.firstTerm !== null
                  ? subjectData.firstTerm
                  : '  -  '}
              </Text>
              <Text style={styles.subjectText}>
                Mid Term:{' '}
                {subjectData.midTerm !== null ? subjectData.midTerm : '  -  '}
              </Text>
              <Text style={styles.subjectText}>
                Final Term:{' '}
                {subjectData.finalTerm !== null
                  ? subjectData.finalTerm
                  : '  -  '}
              </Text>
            </View>
          );
          ...
          <View style={styles.remarksContainer}>
            ...
            <View style={styles.remarksContent}>
              <Text style={styles.remarksText}>
                {capitalizedRemarks ? capitalizedRemarks : '-'}
              </Text>
            </View>
          </View>
        }
